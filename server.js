...
const connectionString =
  'mongodb+srv://amanda-dsilva:<password>@cluster0.w95euv9.mongodb.net/?retryWrites=true&w=majority';

MongoClient.connect(connectionString, { useUnifiedTopology: true })
  .then(client => {
    console.log('Connected to Database')
    const db = client.db('prog-finalproject')
    const accelCollections = db.collection('accel')
    ...
    app.post('/accel', (req, res) => {
      accelCollections.insertOne(req.body)
      .then(result => {
        console.log(result)
        // res.redirect('/')
